...
def _term_set_digest(terms: Iterable[bytes]) -> bytes:
...
class MemoryLedger:
    ...
    def _require_schema(self, connection: sqlite3.Connection) -> dict[str, str]:
        ...
        if metadata.get("ledger_schema") != LEDGER_SCHEMA:
        ...
        if tables != set(TABLE_COLUMNS) or indexes != EXPECTED_INDEXES or other:
    ...
    def _semantic_errors(self, connection: sqlite3.Connection) -> list[str]:
        ...
        for row in event_rows:
            ...
            try:
                expected_terms_digest = _term_set_digest(term_rows)
            except ValueError:
            ...
            for row in candidate_rows[:MAX_RECALL_CANDIDATES]:
                ...
                results.append(
                    self._event_result(
                        connection,
                        row,
                        verified_override=(state == "current"),
                        known_at=known,
                    )
                )
