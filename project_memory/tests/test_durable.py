...
from project_memory import core, durable
from project_memory.durable import MemoryLedger, _eligible_supersessor_exists_sql
from project_memory.tests.fake_redis import FakeRedis
from project_memory.tests.support import PROJECT_ID
...
class DurableMemoryTests(unittest.TestCase):
    ...
    def test_known_at_and_valid_at_are_independent_temporal_axes(self) -> None:
        with mock.patch("project_memory.durable.now_ms", return_value=500):
            session = self.ledger.start_session("temporal recall", session_id="10" * 16)
        with mock.patch("project_memory.durable.now_ms", return_value=1_000):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=3_000):
    ...
    def test_local_api_and_cli_do_not_accept_observation_backdating(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=2_000):
    ...
    def test_session_clock_rollback_clamps_append_and_close_history(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=1_500):
    ...
    def test_evidence_and_initial_verification_clamp_to_event_observation(self) -> None:
        ...
        with mock.patch(
            "project_memory.durable.now_ms", side_effect=[2_000, 1_500, 1_400]
        ):
    ...
    def test_same_millisecond_attempts_use_sequence_not_random_id_order(self) -> None:
        ...
        for name, final_verified, older_id, newer_id in cases:
            with self.subTest(name=name):
                ...
                with mock.patch(
                    "project_memory.durable._new_id", return_value=older_id
                ), mock.patch("project_memory.durable.now_ms", return_value=3_000):
                ...
                with mock.patch(
                    "project_memory.durable._new_id", return_value=newer_id
                ), mock.patch("project_memory.durable.now_ms", return_value=3_000):
    ...
    def test_reverify_clock_rollback_keeps_attempt_history_nondecreasing(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=4_000):
            event = self.ledger.append_event(
                session["session_id"],
                "fact",
    ...
    def test_promotion_visibility_and_flag_are_as_of_known_at(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=1_100):
            event = self.ledger.append_event(
                session["session_id"],
                "decision",
                subject=subject,
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=1_200):
    ...
    def test_cross_session_promotions_remain_valid_as_history_advances(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=1_300):
            self.assertEqual(
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=2_100):
            successor = self.ledger.append_event(
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=2_200):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=2_300):
            self.assertEqual(
    ...
    def test_future_effective_successor_does_not_veto_early_consolidation(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=3_100):
            predecessor = self.ledger.append_event(
                session["session_id"],
                "fact",
                subject=subject,
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=3_150):
            successor = self.ledger.append_event(
                session["session_id"],
                "fact",
                subject=subject,
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=3_200):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=6_000):
    ...
    def test_latest_subject_pointer_is_delivery_metadata_not_current_truth(self) -> None:
        with mock.patch("project_memory.durable.now_ms", return_value=100):
            session = self.ledger.start_session("active pointer semantics")
        subject = "active pointer subject"
        with mock.patch("project_memory.durable.now_ms", return_value=400):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=600):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=300):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=700):
