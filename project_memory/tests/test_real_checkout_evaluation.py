...
from project_memory import cache_v3
from project_memory import core as legacy
...
class FrozenCheckoutEvaluationTests(unittest.TestCase):
    ...
    def test_frozen_fixture_is_excluded_and_cannot_contaminate_results(self) -> None:
        fixture = self.root / "project_memory/evaluation/v25.json"
        ...
        self.assertNotIn("project_memory/evaluation/v25.json", manifest["files"])
        ...
        for outcome in self.result["outcomes"]:
            self.assertNotIn("project_memory/evaluation/v25.json", outcome["returned_paths"])

    def test_frozen_recall_at_five_and_critical_cases_pass(self) -> None:
        fixture = json.loads(
            (self.root / "project_memory/evaluation/v25.json").read_text(encoding="utf-8")
        )
        self.assertEqual(self.result["cases"], len(fixture["cases"]))
