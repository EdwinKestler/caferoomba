...
from project_memory import durable, portable
from project_memory.durable import MemoryLedger, canonical_json
from project_memory.security import ContentFinding
...
class PortableMemoryTests(unittest.TestCase):
    ...
    def test_import_does_not_repeatedly_sort_all_remaining_events(self) -> None:
        ...
        with mock.patch(
            "project_memory.durable.sorted", create=True, side_effect=tracked_sorted
        ):
    ...
    def test_import_preserves_foreign_observation_history_but_checks_at_local_time(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=40_000):
            session = source.start_session("foreign observation history")
        with mock.patch("project_memory.durable.now_ms", return_value=50_000):
            event = source.append_event(
                session["session_id"],
                "fact",
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=60_000):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=10_000):
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=12_000):
    ...
    def test_imported_verification_trust_does_not_travel_before_verified_at(self) -> None:
        ...
        with mock.patch("project_memory.durable.now_ms", return_value=5_000):
            portable.import_pack(target, pack, self.pack_key, activate=True)
        with mock.patch("project_memory.durable.now_ms", return_value=12_000):
