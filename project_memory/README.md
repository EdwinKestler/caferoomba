# Portable Project Memory v2.5.0 bundle
...
`docs/PROJECT_MEMORY_V25_DESIGN.md` and
`docs/PROJECT_MEMORY_V25_RUNBOOK.md`.

## Configuration

A v2.5 repository needs a stable random UUID committed with its slug:
...
```json
{
  ...
  "redis_socket_path": ".project-memory/redis.sock",
  "durable_ledger_path": ".project-memory/memory.sqlite3",
...
python3 project-memory.py status --verbose
...
python3 project-memory.py path "source.name" "target.name" --edge-kind calls
...
and symmetric hot-retrieval p95 at most `1.20x` v2. `clear` deletes only this
...
python3 project-memory.py session-start --task "TASK"
python3 project-memory.py remember --session ID --kind decision \
  --subject "SUBJECT" --payload '{"value":"small typed fact"}' \
  --retention durable \
  --evidence docs/file.md:1-5
python3 project-memory.py session-close ID --outcome "OUTCOME" \
  --evidence docs/file.md:1-5
python3 project-memory.py consolidate ID
python3 project-memory.py recall "QUERY" --promoted-only
...
leave the machine in a pack. Physical pruning of local volatile/session rows
is not implemented in v2.5.
...
```bash
python3 project-memory.py memory-keygen PRIVATE_KEY_FILE.key
python3 project-memory.py memory-export NEW_FILE.pmem --key-file PRIVATE_KEY_FILE.key
python3 project-memory.py memory-import FILE.pmem --key-file PRIVATE_KEY_FILE.key
python3 project-memory.py memory-import FILE.pmem --key-file PRIVATE_KEY_FILE.key --activate --replace
...
configuration, logs, Git or memory records. `PROJECT_MEMORY_PACK_KEY_FILE` may
