# Project Memory v2.5 design

Status: **implemented locally; acceptance evidence pending.** This is
repository tooling, not protocol code and not a D4 public-network phase.

## 1. Outcome

Project Memory v2.5 separates two jobs that v2.4 put in one Redis database:
...
|---|---|---|---|
| Legacy cache | `caferoomba:project-memory:v2` | Redis | Read-only rollback source during migration |
...
| Durable ledger | `project-memory:ledger:v1` | `.caferoomba/project-memory/memory-v25.sqlite3` | Sessions, typed events, temporal history, verification attempts, evidence, workflows and outbox |
...
`project_memory/schemas/`. They document the wire shapes; SQLite constraints
...
machine shape in `project_memory/schemas/workflow-spec.v1.schema.json`.
...
read. `--key-file` is mandatory unless `PROJECT_MEMORY_PACK_KEY_FILE` supplies
...
The separate `scripts/project_memory_redis.sh reset` command operates below
...
Filesystem rollback restores the exact pre-v2.5 tree at commit `88354eb`. It
does not delete, rewrite or export the durable ledger and does not contact
Redis. The operator first checks or exports the ledger, then rolls back code.
The preserved v2 namespace remains available to the restored v2.4 bundle.
