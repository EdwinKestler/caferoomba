...
distribution-provided `python3-cryptography` package; `setup.sh` installs and
...
python3 project-memory.py migration-shadow --limit 5
...
symmetric hot-p95 (`<=1.20x`) gates. It is migration evidence, not the way to
...
```bash
python3 project-memory.py session-start --task "review local Project Memory gate"
```
...
python3 project-memory.py remember \
  ...
  --retention durable \
  --evidence docs/PROJECT_MEMORY_V25_DESIGN.md:1-20
```
...
```bash
python3 project-memory.py recall "Redis disposable projection" --limit 10
python3 project-memory.py recall "Redis disposable projection" --include-history
python3 project-memory.py recall "imported history" --include-untrusted
python3 project-memory.py recall "cache policy" --known-at UNIX_MILLISECONDS --valid-at UNIX_MILLISECONDS
python3 project-memory.py recall "cache policy" --promoted-only
```
...
python3 project-memory.py session-close SESSION_ID \
  --outcome "gate passed" \
  --evidence docs/PROJECT_MEMORY_V25_EVIDENCE.md:1-20
...
```bash
python3 project-memory.py reverify EVENT_ID
```
...
```bash
python3 project-memory.py project-hot --limit 100
python3 project-memory.py ledger-status
```
...
```bash
python3 project-memory.py hot-events --limit 100
python3 project-memory.py hot-events --session SESSION_ID --limit 100
```
...
```bash
python3 project-memory.py project-hot --rebuild --limit 1000
```
...
`project_memory/schemas/workflow-spec.v1.schema.json`. Example:
...
```json
{
  ...
  "steps": [
    {
      "id": "status",
      "action": "python3 project-memory.py status",
...
```bash
python3 project-memory.py workflow-put SESSION_ID local-memory-check WORKFLOW.json \
  --evidence docs/PROJECT_MEMORY_V25_RUNBOOK.md:1-20
python3 project-memory.py workflow-dry-run WORKFLOW_ID
```
...
```bash
python3 project-memory.py memory-keygen \
  .caferoomba/project-memory/transfer-2026-09.key
python3 project-memory.py ledger-status
python3 project-memory.py memory-export \
  /secure/new/location/project-memory.pmem \
  --key-file .caferoomba/project-memory/transfer-2026-09.key
```

`PROJECT_MEMORY_PACK_KEY_FILE` may supply only the key-file path. Never put key
...
```bash
python3 project-memory.py memory-import /secure/location/project-memory.pmem \
...
```bash
python3 project-memory.py memory-recover-import
python3 project-memory.py ledger-status
```
...
```bash
./scripts/project_memory_redis.sh start
python3 project-memory.py project-hot --limit 1000
python3 project-memory.py ledger-status
```
...
```bash
./scripts/project_memory_redis.sh reset
./scripts/project_memory_redis.sh start
python3 project-memory.py index --incremental
python3 project-memory.py project-hot --rebuild --limit 1000
python3 project-memory.py validate --deep
...
backups and `.pmem` packs. Unlike `project-memory.py clear`, it is not a
...
```bash
python3 project-memory.py semantic-status
```
...
```bash
./scripts/project_memory_check.sh
./scripts/rollback_rehearsal.sh
```

The memory gate uses temporary files and an isolated local Redis process or
test double. It must not alter the live ledger, v2 namespace, v3 namespace, or
either local chain.
...
5. Rehearse on a copy with `scripts/rollback_rehearsal.sh`.
6. Apply `python3 scripts/project_memory_v25_rollback.py` only when an actual
   source rollback is intended.

The rollback restores commit `88354eb`. It does not access Redis or the runtime
state directory. The restored v2.4 bundle can use the preserved v2 namespace;
it cannot read the v2.5 SQLite ledger, which remains quarantined for a later
upgrade or explicit export with v2.5 code.

The S2.1 baseline asset `scripts/memory_bootstrap_baseline/.env.example.baseline`
that commit `6dfdcb9` omitted has been tracked since `5fd5709` with the same
SHA-256 (`91f3f2eb7b6596a6d8eb881e998be39ac50f62375c8859b2813e33d19ccb7eca`),
so this stage carries no repair asset; the S2.1 rollback consumes the tracked
copy, and the manual newest-to-oldest chain works outside the rehearsal.
