#!/usr/bin/env bash
# Owner-only Unix-socket Redis for Project Memory.
#
#   ./scripts/project_memory_redis.sh start|stop|status|reset|guard
#
# Redis, reachable only through an owner-mode Unix socket, with its dump, log and pid file under
# `.caferoomba/project-memory/` — a path `.gitignore` already
# excludes. It holds the compact discovery projection that `project-memory.py`
# rebuilds from the checkout; the durable ledger is SQLite, never Redis. The
# dump survives a process restart inside one machine; a fresh checkout starts
# empty and needs `index --incremental`.
#
# A configured CAFEROOMBA_PROJECT_MEMORY_URL, the only variable
# `.project-memory.json` names, still takes precedence over this socket.
# The generic PROJECT_MEMORY_URL is ignored on purpose so another project's
# URL in the same environment cannot select this cache.
set -euo pipefail

cd "$(dirname "$0")/.."

STATE_DIR_REQUEST="${PROJECT_MEMORY_REDIS_DIR:-$PWD/.caferoomba/project-memory}"
SOCKET_PATH_REQUEST="${PROJECT_MEMORY_REDIS_SOCKET:-}"
STATE_DIR=""
PID_FILE=""
LOG_FILE=""
DUMP_FILE="dump.rdb"
...
if [ -z "$REDIS_SERVER" ] || [ -z "$REDIS_CLI" ]; then
  echo "redis-server and redis-cli not found. Run ./setup.sh, which installs the" >&2
  echo "redis-server package, or set REDIS_SERVER_BIN and REDIS_CLI_BIN." >&2
  exit 1
fi
...
def refuse(reason: str) -> None:
    print(f"refusing Project Memory state directory: {reason}", file=sys.stderr)
    raise SystemExit(1)
...
mode = sys.argv[3]
lab_root = os.path.join(repository, ".caferoomba")
try:
...
parts = [".caferoomba", *relative.split(os.sep)]
...
resolve_redis_exe() {
  local path target
  path="$(realpath -e -- "$1" 2>/dev/null || true)"
  [ -n "$path" ] || return 1
  if [ -f "$path" ] && head -c 2 "$path" 2>/dev/null | grep -q '^#!'; then
    target="$(sed -n 's/.*exec[[:space:]]\+"\([^"]*\)".*/\1/p' "$path" | head -1)"
    target="${target/#\$HOME/$HOME}"
    target="${target/#\~/$HOME}"
    realpath -e -- "$target" 2>/dev/null || printf '%s\n' "$path"
  else
    printf '%s\n' "$path"
  fi
}
...
managed_pid() {
  local pid expected_exe actual_exe info_pid configured_socket
  pid="$(live_recorded_pid)" || return 1
  [ "$(stat -c '%u' -- "/proc/$pid" 2>/dev/null || true)" = "$(id -u)" ] || return 1
  expected_exe="$(resolve_redis_exe "$REDIS_SERVER")"
...
start() {
  ...
  if answers; then
    if managed_pid > /dev/null; then
      echo "project-memory redis already running on owner socket $SOCKET_PATH"
    else
  ...
  for _ in $(seq 1 60); do
    if answers && managed_pid > /dev/null; then
      echo "project-memory redis up on owner socket $SOCKET_PATH (state $STATE_DIR)"
      return 0
    fi
    sleep 0.5
  done
  echo "project-memory redis did not come up within 30s; see $LOG_FILE" >&2
  exit 1
}
...
stop() {
  ...
  if pid="$(managed_pid)"; then
    ...
    for _ in $(seq 1 60); do
      if ! kill -0 "$pid" 2>/dev/null; then
        ...
        echo "project-memory redis stopped (dump kept in $STATE_DIR)"
        return 0
      fi
      sleep 0.5
    done
    echo "project-memory redis did not stop within 30s" >&2
    exit 1
  fi
  if [ -e "$PID_FILE" ] || [ -L "$PID_FILE" ]; then
    if ! read_pid_file > /dev/null; then
  ...
  echo "project-memory redis was not running on $SOCKET_PATH"
}
...
status() {
  ...
  if answers; then
    if pid="$(managed_pid)"; then
      echo "project-memory redis: up on owner socket $SOCKET_PATH, pid $pid, state $STATE_DIR"
    else
      echo "project-memory redis: untrusted responder on $SOCKET_PATH (not launcher-owned)" >&2
      return 1
    fi
  ...
  echo "project-memory redis not running on owner socket $SOCKET_PATH"
  return 1
}
